...
speedDistribution <- function(trip)
{
  ...
  vitesse <- rollapply(distance, width = 5, FUN = median) * 3.6 # Smoothing
  
  acceleration_x <- diff(velocity_x) ; acceleration_y <- diff(velocity_y)
  accel <- sqrt(acceleration_x^2 + acceleration_y^2)
  acceleration <- rollapply(accel, width = 5, FUN = median) * 3.6 # Smoothing
  ...
  f5 <- distance2
  feat <- c(f1, f2, f3, f4, f5)
  names(feat) <- paste("f", 1:43, sep="")
  return(feat)    
...
for(driver in drivers)
{
  ...
  g = randomForest(train[,1:43], y = as.factor(train[,44]), ntree = 200)
...
write.csv(submission, "S:/Kaggle/Competition 3/submission3.csv", row.names=F, quote=F)
